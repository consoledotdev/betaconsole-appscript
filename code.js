...
function deployProd() {
    SpreadsheetApp.getUi().alert("Deploying to production is not allowed yet!");
}
...
function deploy(environment) {
    ...
    if (githubToken) {
        var url =
            "https://api.github.com/repos/consoledotdev/betaconsole/actions/workflows/deploy.yml/dispatches";
        var response = UrlFetchApp.fetch(url, {
            contentType: "application/vnd.github.v3+json",
            headers: {
                // Using basic auth with a personal token
                // If we use an OAuth2 app then we have to store the secret in
                // the code. Also tried GitHub Apps but those tokens expire
                // after 10 minutes or 1 hour, and also need the secret in the
                // code
        ...
        if (responseCode == 204) {
            // Doesn't return any content
            if (environment == "test") {
                var deployUrl =
                    "https://betaconsole-test.consoledev.workers.dev/";
            } else {
